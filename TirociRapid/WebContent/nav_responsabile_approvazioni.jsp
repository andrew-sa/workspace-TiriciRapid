
<nav class="navbar navbar-default"
	style="background-color: #f90; margin-top: 20px">
	<div class="container">

		<ul class="nav nav-tabs">

			<li role="presentation"><a class="attiva"
				href="responsabile_approvazioni_richieste_di_tutorato.jsp">Richieste
					di tutorato</a></li>
			<li role="presentation"><a
				href="responsabile_approvazioni_richieste_nuovi_tirocini.jsp">Richieste
					nuovi tirocini</a></li>
			<li role="presentation"><a
				href="responsabile_approvazioni_conferma_finale.jsp">Conferma
					finale</a></li>
			<li role="presentation"><a
				href="responsabile_approvazioni_aziende.jsp">Aziende</a></li>
			<li role="presentation" class="navbar-right"><a href="#">Logout
					&nbsp<i class="fa fa-power-off"></i>
			</a></li>

		</ul>
	</div>

</nav>
