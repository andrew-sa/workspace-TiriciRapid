	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!replaceIfMissing(partitaIVA, "").equals(""))
		{
			...
			try
			{
				...
				String password = managerAzienda.readPassword(partitaIVA);
				inviaEmail(email, password);
				request.setAttribute("successo", "La email di recupero password &egrave; stata inviata con successo");
				RequestDispatcher dispatcher = request.getServletContext().getRequestDispatcher("/index.jsp"); //Alla schermata di LoginAzienda
			...
			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/recupera_password_azienda.jsp"); //Alla schermata di RecuperaPasswordAzienda
			dispatcher.forward(request, response);
		}
		else
		{
